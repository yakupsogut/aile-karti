    ...
    <Layout title="Harcama Ekle">
      <div className="card">
        <div className="form-group">
          ...
          <select className="form-select" value={form.card_id} onChange={e => set('card_id', e.target.value)}>
        ...
        <div className="form-group">
          ...
          <input className="form-input" value={form.description} onChange={e => set('description', e.target.value)} placeholder="örn. Market alışverişi" maxLength={100} />
        </div>

        <div className="form-group">
          <label className="form-label">Tutar (₺) *</label>
          <input className="form-input mono" type="number" min="0.01" step="0.01" value={form.amount} onChange={e => set('amount', e.target.value)} placeholder="0,00" />
        </div>
        ...
        {amount > 0 && (
          <div className="installment-preview">
            {inst === 1
              ...
              : <>Bu ay: <strong>{money(perMonth)}</strong> · Gelecek {inst - 1} ay: <strong>{money(perMonth)} × {inst - 1}</strong></>
            }
          </div>
        )}
        ...
        <button
          ...
          disabled={loading || !form.card_id || !form.assigned_to || !amount}
        >
          {loading ? 'Kaydediliyor...' : 'Harcama Kaydet'}
        </button>
      </div>
    </Layout>
  );
